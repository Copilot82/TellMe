...
документации команды проверены с Xcode 26.2 и Simulator iOS 26.2. Более старый Xcode не считается
поддержанным контуром сборки этого snapshot, даже если отдельные исходники компилируются.
